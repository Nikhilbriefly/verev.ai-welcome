...
const LandingPage = () => {
  ...
  return (
    ...
    {/* hero text */}
      <h1 className="md:text-6xl text-balance text-[23px] sm:text-[23px] mt-5 font-bold bg-gradient-to-r from-zinc-300 via-zinc-500 to-zinc-200 bg-clip-text text-transparent text-center " style={{fontFamily:'clash-display' , fontWeight:200}}  >Write like a pro, even when <br /> you're thinking like you.</h1>
    ...
    {/* email component */}
    <div className="flex gap-2 w-full px-4 items-center mt-8" >
    <input className="bg-zinc-700 z-50 px-4 md:h-12 h-10 bg-opacity-45  border-zinc-700 focus:border-zinc-500 active:border-zinc-600 text-xs md:text-base outline-none placeholder:text-zinc-400 border-2 md:w-[300px] w-full md:rounded-2xl rounded-xl text-white" type="text" placeholder="Enter Your Email" />
    <div className="md:h-11 md:flex hidden h-10 md:px-5 px-2  text-zinc-900 bg-gradient-to-b from-zinc-300 to-zinc-300   border-2 border-zinc-500  md:rounded-xl  items-center justify-center " >
        <p className="md:text-base font-semibold text-xs" >Notify me</p>
    </div>  
    <div className='h-8 w-10  rounded-full flex items-center justify-center bg-gradient-to-b from-white to-zinc-400 md:hidden' >
    ...
    {/* globe component  end */}
      <p className="text-zinc-400 font-light text-sm md:hidden mb-4" >#BuildInPublic</p>
  </div>
  )
}
